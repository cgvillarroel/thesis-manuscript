...
To determine the signer's hand location, the frame will be divided into six sections based on the location of the signer's shoulders. This is because it is the relative positions of the hands that matter, not the absolute cite:[bajernee20]. The presence of hands will be tracked within each bucket throughout the gesture, as seen in <<fig-bucket,{fig-label-bucket}>>, resulting in a vector. The resulting vector from the learner's gesture will then be compared to the reference gesture using a cosine function.
